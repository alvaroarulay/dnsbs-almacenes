...
class ReportselExport implements FromCollection, WithHeadings, WithStyles, WithEvents, WithDrawings, WithCustomStartCell, WithColumnFormatting, WithTitle
{
      // ...
   ...
   public function styles(Worksheet $sheet)
    ...
    public function collection()
    {
        ...
        $entradas = Entradas::join('movimientos','movimientos.id_entrada','=','entradas.id')
        ...
        ->select('partidas.codigo','partidas.nompartida',
            DB::raw('sum(CASE WHEN entradas.saldo_inicial = true THEN entradas.cantidad ELSE (movimientos.cantidad_utilizada+entradas.restante) END) as stock_inicial'),
            DB::raw('sum(entradas.cantidad) as stock_inicial_b'),
            DB::raw('sum(entradas.restante) as stock_final'),
            DB::raw('SUM(entradas.cantidad * entradas.precio_unitario) as total_inicio'),
            DB::raw('sum(entradas.restante * entradas.precio_unitario) as total_final'))
        ->whereBetween('entradas.fecha', [$fechainicial, $fechafinal])
        ->groupBy('partidas.codigo', 'partidas.nompartida')
        ->distinct()
        ->orderBy('partidas.codigo', 'asc')
        ->get();
    ...
    public function headings(): array
    {
        ...
        return [
                'Nro.',
                'Partida',
                'Cantidad Inicial al '.$this->formatfecha($this->fechai),
                'Entradas al '.$this->formatfecha($this->fechaf),
                'Saldo Inicial al 01/01/ (Bs)',
                'Cantidad Final al 31/12/',
                'Saldo Final al 31/12/ (Bs)',
        ];
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $sheet->setCellValue('A5', 'Al 31 de diciembre de la gestión ');
    ...
    protected function formatfecha($value)
    {
